...
class PlannerConsoleFeatureEntryImpl @Inject constructor(): PlannerConsoleFeatureEntry {
    @Composable
    override fun EntryPoint(chatId: Long, modifier: Modifier) {
        ...
        // Use PlannerFactory which injects PlannerTalkDelegate
        val viewModel: ConsoleViewModelImpl = viewModel(
            key = "${ConsoleViewModelImpl::class.qualifiedName}_planner_$chatId",
            factory = featureComponent.getPlannerViewModelFactory(),
            extras = extras
        )
