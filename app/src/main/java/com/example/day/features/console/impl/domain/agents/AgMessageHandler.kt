    ...
    suspend fun handleUserMessage(
        ...
        if (trimmedMessage.startsWith(ChatCommand.Compare.title, ignoreCase = true)) {
            val inputWithoutCommand = trimmedMessage
                ...
                .trimCmd()
            modelsComparator.handle(inputWithoutCommand, chatSettings, tools)
            return
        }
            ...
            if (trimmedMessage.startsWith(command.title, ignoreCase = true)) {
                coroutineScope {
                    worker.doWork(
                        ...
                        chatSettings
                    ).collectWorkerEvents(chatSettings.chatId, tools)
                }
                return
            }
        }

        tools.addBotMessage(chatSettings.chatId, "Команда не распознана")
    }
