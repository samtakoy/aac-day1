...
private fun ChatsScreenInternal(
    ...
    val pageEntry = featureEntries.find<ConsoleFeatureEntry>() as ConsoleFeatureEntryIml
        ...
        if (state.chips.isNotEmpty()) {
            Box(modifier = Modifier.weight(1f)) {
                HorizontalPager(
                    ...
                    val id = state.chips.getOrNull(pageIndex)?.id
                    if (id != null) {
                        pageEntry.PageEntryPoint(id = id, modifier = Modifier.fillMaxSize())
                    }
                }
            }
