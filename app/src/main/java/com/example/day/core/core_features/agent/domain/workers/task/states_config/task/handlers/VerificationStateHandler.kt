...
private fun List<HandlerResult.Message>.addButtons(
    ...
    return this.withButtons(
        buttons = buildList {
            data.stepResults.forEach { step ->
                if (step.score < VerificationStateHandler.Companion.MIN_PASSING_SCORE && data.retryCount < VerificationStateHandler.Companion.MAX_REVIEW_COUNT) {
                    add(
                        ChatMessage.Button(
                            actionId = "${VerificationStateHandler.Companion.ACTION_EXEC}:${step.stepNumber}",
            ...
            add(
                ChatMessage.Button(
                    actionId = VerificationStateHandler.Companion.ACTION_DONE,
                    title = "К результатам",
                    description = "",
