    ...
    override suspend fun doWork(
        ...
        experts.forEach { (name, mission) ->
            ...
            val promptText = "Твоя роль: $name. Задача: $mission. " +
                    if (messageHistory.isNotEmpty()) "Учитывай мнение предыдущих экспертов. Контекст: $userPrompt" else "Контекст: $userPrompt"
        ...
        val managerPrompt = "Ты — Менеджер. Собери воедино мнения аналитика и инженера, " +
                "учти замечания критика и выдай финальный пошаговый план реализации задачи: $userPrompt"
