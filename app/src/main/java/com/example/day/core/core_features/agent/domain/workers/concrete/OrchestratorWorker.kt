...
10. Если подзадача читает данные только чтобы передать их следующему шагу — объедини чтение и обработку в один шаг. Промежуточные сырые данные (содержимое файлов и т.п.) не должны быть результатом отдельного шага.
...
13. Если подзадача сравнивает два источника данных (файл A vs файл B, ветка X vs ветка Y), явно укажи что является базой (reference), а что — изменённой версией (current). В тексте подзадачи используй формулировку: "Присутствует в CURRENT, отсутствует в BASE: ..." и "Присутствует в BASE, отсутствует в CURRENT: ...". Это исключает путаницу в интерпретации "добавлено/удалено".
14. В начале каждой подзадачи ОБЯЗАТЕЛЬНО указывай строку depends_on: [номера шагов] — список шагов (1-based), результаты которых нужны этому шагу. Исполнитель получит ТОЛЬКО результаты указанных шагов. Примеры: depends_on: [] — шаг независим; depends_on: [1] — нужен результат шага 1; depends_on: [1, 3] — нужны результаты шагов 1 и 3.
