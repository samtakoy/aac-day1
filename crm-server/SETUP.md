...
## 5. Debug endpoints (просмотр из консоли)

Удобные GET-эндпоинты для ручного дебага БД.

### Пользователи

```bash
# Найти пользователей по имени (подстрока, case-insensitive)
curl "http://localhost:3002/debug/users?name=Иван"

# Найти пользователя по chatId
curl "http://localhost:3002/debug/users?chatId=123456789"
```

Пример ответа (`?name=Иван`):
```json
[
  {"id": 1, "chatId": 123456789, "name": "Иван Петров"},
  {"id": 3, "chatId": 987654321, "name": "Иванов Сергей"}
]
```
...
```bash
# Все тикеты пользователя по chatId
curl "http://localhost:3002/debug/tickets?chatId=123456789"

# Все тикеты с заданным статусом: open | closed | operator
curl "http://localhost:3002/debug/tickets?status=open"

# Все тикеты в БД
curl "http://localhost:3002/debug/tickets"
```
...
[
  {
    ...
    "description": "При входе выдаёт ошибку 401",
    "result": ""
  }
]
```
