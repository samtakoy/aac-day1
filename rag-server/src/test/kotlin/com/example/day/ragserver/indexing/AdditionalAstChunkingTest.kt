...
class AdditionalAstChunkingTest {
    ...
    fun testFunctionModifiers() {
        ...
        // concat (53 chars) exceeds maxChunkSize=30 → split into parts; check base name
        assertTrue(chunks.any { it.declarationName?.substringBefore("#") == "concat" }, "Expected infix function")
    }
    ...
    fun testExtensionFunctions() {
        ...
        // Both extension functions (43+ chars) exceed maxChunkSize=30 → split; check base names
        assertTrue(chunks.any { it.declarationName?.substringBefore("#") == "reversed" }, "Expected reversed extension function")
        assertTrue(chunks.any { it.declarationName?.substringBefore("#") == "square" }, "Expected square extension function")
    }
    ...
    fun testAnnotations() {
        ...
        // oldMethod has two annotations in its node text (~68 chars) → exceeds maxChunkSize=30 → split.
        // @Deprecated lands in part1, fun declaration in the last part. Check by base name.
        val methodChunks = chunks.filter { it.declarationName?.substringBefore("#") == "oldMethod" }
        assertTrue(methodChunks.isNotEmpty(), "Expected oldMethod chunk(s)")
        assertTrue(methodChunks.any { it.content.contains("@Deprecated") }, "Expected @Deprecated annotation")
    ...
    fun testKDocComments() {
        ...
        // documentedFunction: KDoc + fun body ~50 chars > maxChunkSize=30 → split into parts.
        // KDoc lands in part1. Check all parts together.
        val funcChunks = chunks.filter { it.declarationName?.substringBefore("#") == "documentedFunction" }
        assertTrue(funcChunks.isNotEmpty(), "Expected documentedFunction chunk(s)")
        assertTrue(funcChunks.any { it.content.contains("Single line KDoc") }, "Expected KDoc in function chunk")
    }
    ...
    fun testLargeFunctionSplitIntoParts() {
        ...
        // huge() is split into parts; there is no unsplit chunk with declarationName == "huge".
        // Verify the base name exists in at least one part.
        assertTrue(
            chunks.any { it.declarationName?.substringBefore("#") == "huge" },
            "Expected chunk(s) for huge"
        )
        assertTrue(
    ...
    fun testOversizedKDocGetsSeparated() {
        ...
        val chunks = strategy.split(code, "test.kt", "test.kt")
        // KDoc (~200 chars) + fun body > maxChunkSize=30 → split into many #partN chunks.
        // Filter by base name (substringBefore("#")) to collect all parts.
        val docHeavyChunks = chunks.filter { it.declarationName?.substringBefore("#") == "docHeavy" }
        ...
        assertTrue(docHeavyChunks.size >= 2, "Expected multiple split chunks for docHeavy")
        assertTrue(docHeavyChunks.any { it.content.contains("/**") }, "Expected a chunk with KDoc start")
        assertTrue(docHeavyChunks.any { it.content.contains("fun docHeavy()") }, "Expected a chunk with function declaration")
    }
}
