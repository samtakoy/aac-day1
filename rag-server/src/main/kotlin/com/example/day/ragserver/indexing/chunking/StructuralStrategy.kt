...
class StructuralStrategy(
    ...
    override fun split(content: String, filePath: String, fileName: String): List<ChunkEntity> {
        ...
        for ((index, block) in blocks.withIndex()) {
            ...
            val nodeType = inferNodeType(block)
            ...
            if (blockWithHeader.length <= maxChunkSize) {
                chunks.add(
                    ChunkEntity(
                        ...
                        startLine = startLine, nodeType = nodeType,
    ...
    /**
     * Приближённо определяет тип AST-узла из первой строки блока.
     * Используется для заполнения [ChunkEntity.nodeType] без AST-парсинга.
     * Точность достаточна для фильтрации top-level деклараций в IndexingService.
     */
    private fun inferNodeType(block: String): String? {
        val firstLine = block.trimStart().lines().firstOrNull { it.isNotBlank() } ?: return null
        val trimmed = firstLine.trim()
        return when {
            // interface — в Kotlin grammar это class_declaration с модификатором
            trimmed.contains(Regex("""(?:^|\s)interface\s""")) -> "class_declaration"
            // companion object
            trimmed.contains(Regex("""companion\s+object""")) -> "object_declaration"
            // object
            trimmed.contains(Regex("""(?:^|\s)object\s""")) -> "object_declaration"
            // class variants: data class, sealed class, abstract class, enum class, class
            trimmed.contains(Regex("""(?:^|\s)class\s""")) -> "class_declaration"
            // fun
            trimmed.contains(Regex("""(?:^|\s)fun\s""")) -> "function_declaration"
            // typealias
            trimmed.contains(Regex("""(?:^|\s)typealias\s""")) -> "type_alias"
            else -> null
        }
    }
