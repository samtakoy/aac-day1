...
**`AIAgent`** [ИСТОЧНИК 1] — основной класс агента, поддерживающий как простые single-run агенты, так и сложные workflow-агенты. Создание агента:
...
**Complex workflow agents** [ИСТОЧНИК 2] позволяют создавать сложные стратегии с множеством узлов и рёбер:
...
1. **Tool calling** — агенты могут вызывать инструменты через LLM [ИСТОЧНИК 3]
2. **Custom strategies** — определение собственных workflow через DSL [ИСТОЧНИК 2]
3. **Features** — расширяемость через EventHandler, Tracing, AgentMemory [ИСТОЧНИК 4]
4. **Event handling** — подписка на события жизненного цикла агента [ИСТОЧНИК 5]

### Источники

[ИСТОЧНИК 1] https://github.com/jetbrains/koog-docs/blob/main/docs/single-run-agents.md

[ИСТОЧНИК 2] https://github.com/jetbrains/koog-docs/blob/main/docs/complex-workflow-agents.md

[ИСТОЧНИК 3] https://github.com/jetbrains/koog-docs/blob/main/docs/key-concepts.md

[ИСТОЧНИК 4] https://github.com/jetbrains/koog-docs/blob/main/docs/agent-memory.md

[ИСТОЧНИК 5] https://github.com/jetbrains/koog-docs/blob/main/docs/tracing.md
...
**2. Tool Registry** — реестр доступных инструментов [ИСТОЧНИК 1]:
```kotlin
...
**3. Agent Config** — общая конфигурация [ИСТОЧНИК 2]:
```kotlin
...
**4. Strategy** — стратегия выполнения [ИСТОЧНИК 3]:
```kotlin
...
**5. Install Features** — установка дополнительных возможностей [ИСТОЧНИК 4]:
```kotlin
installFeatures = {
    install(EventHandler) {
...
[ИСТОЧНИК 1] https://github.com/jetbrains/koog-docs/blob/main/docs/built-in-tools.md

[ИСТОЧНИК 2] https://github.com/jetbrains/koog-docs/blob/main/docs/complex-workflow-agents.md

[ИСТОЧНИК 3] https://github.com/jetbrains/koog-docs/blob/main/docs/custom-strategy-graphs.md

[ИСТОЧНИК 4] https://github.com/jetbrains/koog-docs/blob/main/docs/opentelemetry-support.md
...
Агент работает с историей через **LLM Sessions** и **History Compression Strategies** [ИСТОЧНИК 1].

### Стратегии сжатия истории

**`HistoryCompressionStrategy`** позволяет управлять историей сообщений [ИСТОЧНИК 2]:
...
**`WholeHistory`** — суммирует всю историю в TL;DR [ИСТОЧНИК 3]:
```kotlin
llm.writeSession {
...
[ИСТОЧНИК 1] https://github.com/jetbrains/koog-docs/blob/main/docs/sessions.md

[ИСТОЧНИК 2] https://github.com/jetbrains/koog-docs/blob/main/docs/history-compression.md

[ИСТОЧНИК 3] https://github.com/jetbrains/koog-docs/blob/main/docs/nodes-and-components.md

---

## Q4: "как реализованы стратегии компактизации контекста"

Стратегии компактизации контекста реализованы через **History Compression Strategies** [ИСТОЧНИК 1]:

### Доступные стратегии

1. **`FromLastNMessages(n)`** — сохраняет только последние n сообщений [ИСТОЧНИК 2]

2. **`WholeHistory`** — заменяет всю историю одним TL;DR резюме [ИСТОЧНИК 3]
...
[ИСТОЧНИК 1] https://github.com/jetbrains/koog-docs/blob/main/docs/history-compression.md

[ИСТОЧНИК 2] https://github.com/jetbrains/koog-docs/blob/main/docs/nodes-and-components.md

[ИСТОЧНИК 3] https://github.com/jetbrains/koog-docs/blob/main/docs/sessions.md
...
**Tool** — это функция, которую агент может использовать для выполнения задач или взаимодействия с внешними системами. **Tool call** — запрос LLM на выполнение инструмента [ИСТОЧНИК 1].
...
События tool calling отслеживаются через Tracing [ИСТОЧНИК 2]:
...
[ИСТОЧНИК 1] https://github.com/jetbrains/koog-docs/blob/main/docs/key-concepts.md

[ИСТОЧНИК 2] https://github.com/jetbrains/koog-docs/blob/main/docs/tracing.md
...
Koog предоставляет встроенные инструменты для user in the loop [ИСТОЧНИК 1]:
...
Агенты работают с памятью через **AgentMemory** feature [ИСТОЧНИК 1]:
...
Система использует **MemoryScopes** для организации данных [ИСТОЧНИК 2]:
...
[ИСТОЧНИК 1] https://context7.com/jetbrains/koog-docs/llms.txt

[ИСТОЧНИК 2] https://github.com/jetbrains/koog-docs/blob/main/docs/agent-memory.md
...
Koog поддерживает **MCP** для интеграции с внешними MCP-серверами [ИСТОЧНИК 1]:
...
[ИСТОЧНИК 1] https://github.com/jetbrains/koog-docs/blob/main/docs/model-context-protocol.md
...
**Subgraph** — это самодостаточная единица обработки внутри стратегии агента, с собственным набором инструментов, контекстом и ответственностью [ИСТОЧНИК 1].
...
1. **Модульность** — каждый subgraph может иметь свой набор инструментов [ИСТОЧНИК 2]
2. **Изоляция** — собственный контекст выполнения
3. **Переиспользование** — можно использовать одни subgraph в разных стратегиях

### Источники

[ИСТОЧНИК 1] https://github.com/jetbrains/koog-docs/blob/main/docs/subgraphs-overview.md

[ИСТОЧНИК 2] https://github.com/jetbrains/koog-docs/blob/main/docs/custom-subgraphs.md
...
В Koog агенты могут взаимодействовать через **subgraphs** в рамках одной стратегии, где результат одного subgraph передаётся в другой [ИСТОЧНИК 1]:
...
Агенты могут обмениваться информацией через общую память [ИСТОЧНИК 2]:
...
[ИСТОЧНИК 1] https://github.com/jetbrains/koog-docs/blob/main/docs/custom-strategy-graphs.md
...
Агент работает с LLM через **sessions**, которые позволяют управлять историей и параметрами [ИСТОЧНИК 1].
...
**`AIAgentLLMSession`** — интерфейс сессии [ИСТОЧНИК 2]:
...
[ИСТОЧНИК 2] https://api.koog.ai/agents/agents-core/ai.koog.agents.core.agent.context/-a-i-agent-l-l-m-context/index.html
...
- **Subgraph context** — каждый subgraph имеет доступ к состоянию через контекст [ИСТОЧНИК 1]
...
Обработка ошибок реализована через события в Tracing [ИСТОЧНИК 1]:
...
[ИСТОЧНИК 1] https://github.com/jetbrains/koog-docs/blob/main/docs/tracing.md
...
1. **User in the loop** — использование `AskUser` для подтверждения опасных действий [ИСТОЧНИК 1]
...
Диалог хранится в **LLM Session** как история сообщений [ИСТОЧНИК 1]:
...
**Tracing** — основной механизм observability [ИСТОЧНИК 1]:
...
**`GraphAIAgent`** — это агент, который использует **graph-based strategy** для определения workflow через nodes и edges [ИСТОЧНИК 1].
...
[ИСТОЧНИК 1] https://github.com/jetbrains/koog-docs/blob/main/docs/custom-subgraphs.md
...
**`AIAgentContext`** предоставляет доступ к окружению агента во время выполнения [ИСТОЧНИК 1].
...
**`AIAgentLLMContext`** — интерфейс для работы с LLM в рамках сессии агента [ИСТОЧНИК 1].
...
> "Sessions are created using extension functions on the `AIAgentLLMContext` class. These functions take a lambda block that runs within the context of the session." [ИСТОЧНИК 2]

### Источники

[ИСТОЧНИК 1] https://api.koog.ai/agents/agents-core/ai.koog.agents.core.agent.context/-a-i-agent-l-l-m-context/index.html

[ИСТОЧНИК 2] https://github.com/jetbrains/koog-docs/blob/main/docs/sessions.md
...
[ИСТОЧНИК 1] https://github.com/jetbrains/koog-docs/blob/main/docs/complex-workflow-agents.md
...
**`AIAgentGraphStrategy`** — стратегия, основанная на графах, где workflow определяется как набор **nodes** (узлов) и **edges** (рёбер) [ИСТОЧНИК 1].
...
**Node** — это фундаментальный строительный блок в workflow агента, представляющий операцию или преобразование данных [ИСТОЧНИК 1].
...
Для более продвинутого state management доступен snapshot mechanism [ИСТОЧНИК 2]:
```kotlin
...
[ИСТОЧНИК 1] https://github.com/jetbrains/koog-docs/blob/main/docs/agent-memory.md

[ИСТОЧНИК 2] https://github.com/jetbrains/koog-docs/blob/main/docs/agent-persistency.md
...
**`LocalFileMemoryProvider`** использует `SimpleStorage` для файлового хранилища [ИСТОЧНИК 1]:
```kotlin
val memoryProvider = LocalFileMemoryProvider(
    config = LocalMemoryConfig("my-agent-memory"),
    storage = SimpleStorage(JVMFileSystemProvider.ReadWrite),
...
**Strategy** определяет **что** делает агент и **как** он это делает — структуру workflow через nodes и edges [ИСТОЧНИК 1].
...
**Subgraph** — это **самостоятельная единица обработки** внутри стратегии с собственным набором инструментов и контекстом [ИСТОЧНИК 1].
...
**ExecutionPointNode** (nodeStart, nodeFinish) — это специальные узлы, определяющие **начало и конец** выполнения subgraph или стратегии [ИСТОЧНИК 1].
...
**setExecutionPoint** позволяет установить произвольную точку выполнения [ИСТОЧНИК 2]:
```kotlin
context.persistency().setExecutionPoint(
    agentContext = context,
    nodeId = "target-node-id",
...
[ИСТОЧНИК 1] https://github.com/jetbrains/koog-docs/blob/main/docs/testing.md
...
**`subgraph`** — это DSL функция для построения subgraphs [ИСТОЧНИК 1].
...
Готовый builder для типичных задач [ИСТОЧНИК 2]:
```kotlin
val processQuery by subgraphWithTask<String>(
...
(нет достаточного контекста в текущей выборке)
